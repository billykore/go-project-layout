...
```
cmd/_your_app_/main.go          ← Entrypoint: wires all dependencies
...
9. **Wire everything** in `cmd/_your_app_/main.go`.
...
- Use `go test -v ./...` to run all tests (note: directories starting with `_` like `_your_app_` may need explicit paths for `go test`).
